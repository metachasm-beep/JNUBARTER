    ...
    <div className="min-h-screen bg-stone-50 flex">
      ...
      <aside className="w-64 bg-white border-r border-stone-200 flex flex-col fixed h-full z-50">
        ...
        <nav className="flex-1 px-4 space-y-2 pt-8">
          ...
          <AdminNavLink href="/admin/users" icon={<Users className="h-4 w-4" />} label="Peer Network" />
          <AdminNavLink href="/admin/listings" icon={<Package className="h-4 w-4" />} label="Registry Moderation" />
          <AdminNavLink href="/admin/logs" icon={<Activity className="h-4 w-4" />} label="System Pulse" />
        </nav>

        <div className="p-4 border-t border-stone-100">
           <Link href="/" className="flex items-center gap-3 p-4 rounded-2xl text-stone-400 hover:text-primary transition-colors">
...
function AdminNavLink({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <Link 
      href={href}
      className="flex items-center gap-4 px-6 py-4 rounded-2xl text-stone-400 hover:bg-stone-50 hover:text-primary transition-all group"
    >
