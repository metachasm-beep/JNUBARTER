...
export function FAQSection() {
  return (
    <section id="faq" className="w-full py-40 bg-[#FDFCFB] px-8 relative overflow-hidden border-t border-stone-200">
      <div className="max-w-4xl mx-auto space-y-16">
        <div className="space-y-6 text-center">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-5xl md:text-7xl font-black tracking-tighter text-[#1B1B1B] uppercase font-serif italic">Institutional FAQ</h2>
            <p className="text-stone-500 font-medium tracking-tight mt-4 text-lg italic">layman's guide to the JNU scholarly reciprocity protocol.</p>
          </motion.div>
        </div>
        ...
        >
          <Accordion className="w-full space-y-4">
            {FAQ_DATA.map((item, i) => (
              <AccordionItem key={i} value={`item-${i}`} className="border-stone-200 bg-white rounded-3xl px-8 overflow-hidden transition-all hover:border-stone-300 shadow-sm">
                <AccordionTrigger className="text-left text-sm md:text-base font-bold uppercase tracking-tight text-[#1B1B1B] hover:no-underline hover:text-accent transition-all py-6 font-sans border-none">
                  {item.q}
                </AccordionTrigger>
                <AccordionContent className="text-[#333333] leading-relaxed font-medium pt-2 pb-8 text-base md:text-lg border-t border-stone-50">
                  <motion.div
